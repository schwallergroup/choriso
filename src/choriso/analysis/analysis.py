...
def compute_properties(
    ...
    react_mols = df['main_reactant'][df['main_reactant'].apply(is_valid_smiles)].apply(Chem.MolFromSmiles)
...
def log_dataset_info(
    ...
    logger.log('\nPRODUCTS\n')
    distrib_prods = (df["n_products"]
                    .value_counts()
                    .reset_index()
                    .rename(columns={"n_products": "num_reactions",
                                    "index": "num_products_per_reaction"})
                )
    distrib_prods = distrib_prods.set_index("num_products_per_reaction")
    distrib_prods["relative"] = distrib_prods["num_reactions"]*100/distrib_prods["num_reactions"].sum()
    ...
    logger.log('\nNumber of reactions with stereocenters: {0} ({1}% of the total)'.format(stereos, stereos/len(df)*100))
    logger.log('\n\n')


def log_reactions_info(
        df,
        logger,
        n_rxns=100,
        seed=42
    ...
    for idx, row in df.sample(n_rxns, random_state=seed).iterrows():
        ...
        ax[0].set_title(
            f"Test idx: {idx}, NameRXN: {row['rxn_class']}",
            fontsize=50
        )
