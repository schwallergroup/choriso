...
def df_cleaning_step(
   ...
   if name == "cjhif":
      ...
      df = preproc.preprocess_additives(data_dir,
                                        raw_file_name,
                                        "cjhif",
                                        logger)

      #Create full reaction SMILES
      df = preproc.get_full_reaction_smiles(
         df,
         "cjhif",
         logger
      )
   ...
   if name == "uspto":
      ...
      df = (
         pd.read_csv(data_dir + raw_file_name,
                     sep="\t",
                     usecols=["ReactionSmiles","CalculatedYield"],
                     ).rename(
                        columns={"ReactionSmiles":"full_reaction_smiles",
   ...
   df.to_csv(out_dir + f'{name}_processed_clean.tsv',
             sep='\t',
             index=False)
...
def df_atom_map_step(
   ...
   if not os.path.isfile(filepath):
      raise Exception("Atom mapping step couldn't be started. Make sure the cleaning step finished successfully.")
   ...
   df = atom_map.atom_map_rxnmapper(
      df,
      "full_reaction_smiles",
      name,
      logger
   ...
   (df
    ...
    .to_csv(out_dir + "errors_nmrxn.tsv",
            index=False,
            sep="\t")
    )
   ...
   df["nm_aam"].replace("pailas socio",
                        np.nan,
                        inplace=True)
   ...
   df["nm_aam"].fillna(df["rxnmapper_aam"],
                       inplace=True)
   ...
   if name=="cjhif":
      ...
      choriso.to_csv(out_dir + f'choriso.tsv',
                     sep='\t',
                     index=False)
   ...
   df.to_csv(out_dir + f'{name}_atom_mapped_dataset.tsv',
             sep="\t",
             index=False)


def df_analysis_step(
      data_dir,
      report_dir,
      out_dir,
      logger
   ...
   def _get_properties_dataset(out_dir, df, ds_name="uspto"):
      ...
      if ds_name=="uspto":
         ...
         rxn_col = "rxn_smiles"
      df_prop = analysis.compute_properties(
         df,
         rxn_col
      )
      df_prop.to_csv(out_dir + f'{ds_name}_properties.csv',
                     sep='\t',
                     index=False)
      return df_prop
   ...
   def _load_datasets():
      ...
      if os.path.isfile(out_dir + "cjhif_atom_mapped_dataset.tsv"):
         cjhif = pd.read_csv(
            out_dir + "cjhif_atom_mapped_dataset.tsv",
            sep="\t"
         )
         aam_cjhif = True
      else:
      ...
      if os.path.isfile(out_dir + "uspto_atom_mapped_dataset.tsv"):
         uspto = pd.read_csv(
            out_dir + "uspto_atom_mapped_dataset.tsv",
            sep="\t"
         )
         aam_uspto = True
      else:
   ...
   if uspto_check and choriso_check:
      uspto_prop = pd.read_csv(out_dir + "uspto_properties.csv",
            sep="\t"
         )
      cjhif_prop = pd.read_csv(out_dir + "cjhif_properties.csv",
            sep="\t"
         )
      choriso_prop = pd.read_csv(out_dir + "choriso_properties.csv",
            sep="\t"
         )
   ...
   for df, name in [(uspto_prop, 'uspto'),
                    (cjhif_prop, 'cjhif'),
                    (choriso_prop, "choriso")]:
      plot.top_k_products(
         df,
         20,
         name,
   ...
   if aam_cjhif & aam_uspto:
      ...
      rxn_class_map = {'0':'Unassigned', '1':'Heteroatom alkylation/arylation', '2':'Acylation',
                  '3':'C-C bond formation', '4':'Heterocycle formation', '5':'Protection',
                  '6':'Deprotection', '7':'Reduction', '8':'Oxidation', '9': 'FGI', '10':'FGA',
                  '11':'Resolution', '12':'Miscelaneous', 'nan': 'nan'}
      ...
      choriso_rxn_lvls = analysis.distrib_reaction_types(
         choriso["rxn_class"],
         "choriso"
      )
...
def df_splitting_step(
   ...
   if mode == 'mw':
      ...
      processing.split.data_split_random(
         path,
         out_dir,
         test_frac=0.07,
         val_frac=0.077
...
@click.option(
   "--wandb_log",
   is_flag=True,
   help="Log results using Weights and Biases."
)
@click.option(
   "--uspto",
   is_flag=True,
...
def main(
   ...
   if "analysis" in run:
      df_analysis_step(data_dir,
                       report_dir,
                       out_dir,
                       logger)

   if "split" in run:
      df_splitting_step(out_dir,
                        out_dir,
                        split_mode,
                        low_mw,
                        high_mw,
