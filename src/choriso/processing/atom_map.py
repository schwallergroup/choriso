...
def clean_preproc_df(
    ...
    query = suspect[suspect[col].apply(
        rxn_utils.has_more_than_max_tokens,
        rxnmapper = rxnmapper
    )].index
...
def atom_map_hazelnut(
    ...
    for i, df_i in enumerate(tqdm(df_batches)):
        try:
            ...
            mapped_i = pd.read_csv(mapped_file,
                                   sep=" ",
                                   header=None)
...
def atom_map_rxnmapper(
    ...
    def _atom_map(df, col, batch_size=batch_size):
        map_rxn = []
        for batch in tqdm(gen_batches(df.shape[0],
                                      batch_size),
    ...
    aam_rxns = (
        _atom_map(df, col)
        .rename(
            columns={'mapped_rxn':f"rxnmapper_aam",
...
def aam_reagent_classify(smi):
    ...
    # Classify
    return sorted([rxn_utils.canonical_smiles_mol(r)
                   for r in reactsl if _is_reagent(r)])
